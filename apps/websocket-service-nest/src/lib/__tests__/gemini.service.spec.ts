...
let mockSendMessageImpl: () => Promise<any> = () => Promise.resolve({ response: mockResponse });
...
describe('GeminiService', () => {
  ...
  describe('generateAIResponseWithFunctions', () => {
    ...
    it('should handle register_user function call', async () => {
      ...
      const messages = [
        { text: 'Meu nome é João Silva, tenho um problema trabalhista urgente', sender: 'user' },
      ];
    ...
    it('should handle update_conversation_status function call', async () => {
      ...
      const messages = [
        { text: 'Fui demitido sem justa causa, preciso de ajuda', sender: 'user' },
      ];
    ...
    it('should handle multiple function calls', async () => {
      ...
      const messages = [
        { text: 'Olá, preciso de uma consulta jurídica simples', sender: 'user' },
      ];
      ...
      expect(result.functionCalls![1].name).toBe('detect_conversation_completion');
    });
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(service.generateAIResponseWithFunctions(messages)).rejects.toThrow('API Error');
  ...
  describe('updateSystemPrompt', () => {
    it('should log deprecation message', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Use AIService.updateConfig() para atualizar o prompt do sistema');
