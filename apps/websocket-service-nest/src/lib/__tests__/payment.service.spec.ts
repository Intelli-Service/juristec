...
describe('PaymentService', () => {
  ...
  beforeEach(async () => {
    ...
    const mockPaymentModel = {
      new: jest.fn().mockImplementation((data) => ({
        ...mockPayment,
        ...data,
        save: jest.fn().mockResolvedValue({ ...mockPayment, ...data }),
      })),
      findById: jest.fn(),
      find: jest.fn(),
      findOne: jest.fn(),
  ...
  describe('createPayment', () => {
    ...
    it('should throw BadRequestException when card data is missing for credit card', async () => {
      ...
      (conversationModel.findOne as jest.Mock).mockResolvedValue(
        mockConversation,
      );
  ...
  describe('processWebhook', () => {
    it('should process payment webhook successfully', async () => {
      ...
      (paymentModel.findOne as jest.Mock).mockResolvedValue(freshPayment);
      (transactionModel.updateOne as jest.Mock).mockResolvedValue({
        acknowledged: true,
      });
  ...
  describe('refundPayment', () => {
    ...
    it('should throw BadRequestException when payment is not paid', async () => {
      ...
      (paymentModel.findById as jest.Mock).mockResolvedValue(pendingPayment);
  ...
  describe('getPaymentsByLawyer', () => {
    it('should return payments for a lawyer', async () => {
      ...
      (paymentModel.find as jest.Mock).mockReturnValue(mockQuery);
