  ...
  try {
    ...
    if (!session || session.user.role !== 'client') {
      return NextResponse.json(
        { error: 'Acesso negado. Apenas clientes podem aceitar cobranças.' },
        { status: 403 }
      );
    }
    ...
    const response = await fetch(`${backendUrl}/billing/accept-charge`, {
      method: 'POST',
      headers: {
    ...
    console.error('Erro ao aceitar cobrança:', error);
    return NextResponse.json(
