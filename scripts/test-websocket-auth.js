...
function testUnauthenticatedConnection() {
  return new Promise((resolve) => {
    ...
    socket.on('connect', () => {
      ...
      console.log(`⚠️ Conectado sem autenticação em ${results.connectionTime}ms`);

      // Aguardar um pouco para ver se será desconectado
      setTimeout(() => {
        if (!results.rejected) {
          console.log('Cliente ainda conectado após 2s - comportamento inesperado');
        }
      }, 2000);
    });
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (reason === 'io server disconnect' || reason === 'io client disconnect') {
        console.log(`🔌 Desconectado pelo servidor após ${totalTime}ms: ${reason}`);
        results.rejected = true;
      } else {
        console.log(`🔌 Desconectado após ${totalTime}ms: ${reason}`);
      }
    });
...
async function runAllTests() {
  ...
  try {
    ...
    const unauthRejected = unauthResults.rejected && unauthResults.disconnectReason === 'io server disconnect';
    const invalidRejected = invalidResults.rejected && invalidResults.disconnectReason === 'io server disconnect';

    console.log(`🔐 Autenticação funciona: ${authSuccess ? '✅ SIM' : '❌ NÃO'}`);
    console.log(`🚫 Rejeição sem auth: ${unauthRejected ? '✅ SIM' : '❌ NÃO'} (${unauthResults.connectionTime}ms)`);
    ...
    if (authSuccess && unauthRejected && invalidRejected) {
      ...
      console.log('✅ Usuários não autenticados são rejeitados imediatamente');
      console.log('✅ Tokens inválidos são rejeitados imediatamente');
